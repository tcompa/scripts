...
description:   Computes g(r) for a set of configurations of N three-dimensional
               points uniformly distributed in a cube.
...
def compute_gr_3d(samples, N, nbins=100):
    '''
    ...
    gr = prob_r * N / (4.0 * numpy.pi * r ** 2)
    return r, gr


N = 100                    # number of points
L = 1000.0 ** (1.0 / 3.0)  # edge of the cube
rho = float(N) / L ** 3    # average density
...
# compute g(r)
r, gr = compute_gr_3d(samples, N, 100)
...
plt.savefig('fig_gr_3d.pdf', bbox_inches='tight')
plt.show()
